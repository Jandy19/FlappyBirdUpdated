...
class bGame extends Game {
  ...
  void render(Canvas canvas){
    if(background==null){
      background = Background(screenSize.width,screenSize.height);
    }
    if(!lost){
      var rng = Random();
      ...
      if(rng.nextInt(100) < 5) {
        ...
        if(flag == 1) {
          pipes.add(Pipe(screenSize.height,screenSize.width));
        }
      }
      ...
      // Displaying the pipe
      for (Pipe x in pipes){
        x.render(canvas,paint,index,cancer,screenSize.height);
      }
  ...
  void update(double t){
    ...
    if(start){
      ...
      if(player.getY() < screenSize.height) {
        player.fall();
      }else{
        lost = true;
      }
  ...
  getCost(int index) async{
    ...
    if(index == 1){
      if(prefs.getBool('ownTrump') == false) {
      ...
      if(prefs.getBool('ownNyanCat') == false) {
